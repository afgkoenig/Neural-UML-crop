...
def main():
    ...
    Path_img = "./pre_traitement/test_700/"
    Path_label = "./pre_traitement/labels_700/"
    global_count = 1

    
    with open('label_700.csv', newline='') as csvfile:
        ...
        for row in filereader:
            ...
            if row[1] == "Yes":
                ...
                img = Image.open(f"pre_traitement/700/{name}")
                ...
                for elt in row:
                    ...
                    im_crop.save(Path_img+f"{global_count}.jpg")
                    ...
                    with open(Path_label+f"{global_count}.txt","w") as file:
                                file.write(texte)
    ...
    Path_test = f"./pre_traitement/test_700/" # image to visualize
    pred = model.predict(Path_test,save=True,save_txt = True,conf=0.3,save_conf=True,device = 0)  # max_det = 1

    try :
        os.rename("./runs/detect/predict/",f"./runs/detect/predict{700}/")
    except : FileExistsError
    

    for files in os.listdir(f"./pre_traitement/labels_700/"):
        with open(f"./pre_traitement/labels_700/"+files, "r") as filereader:
            for row in filereader:
                row = row.split(" ")
                try:
                    with open(f"./runs/detect/predict{700}/labels/"+files, "r") as filereader2:
                        ...
                        for elt in row2:
                            ...
                            if int(row[0]) == int(elt[0]): #if we have the same label we calculate the f1_score and the recovery percentage
                                shape = np.shape(Image.open(f"./pre_traitement/test_700/"+files[:-4]+".jpg"))
    ...
    cm_kfold_analysis([confusion_matrix_normalized],classes=["navigable","inheritance","realization","dependency","aggregation","composition","classe"],filename=f"confusion_matrix_700")
